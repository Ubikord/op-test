...
# ============================================================
echo -e "${BLUE}[2/8] Подготовка кода...${NC}"
...
# ============================================================
echo -e "${BLUE}[4/8] Создание topology.json...${NC}"
...
# ============================================================
# ШАГ 5: Настройка статического IP для связи с R2S
# ============================================================
echo -e "${BLUE}[5/8] Настройка статического IP на enP3p49s0...${NC}"

# Проверяем, существует ли интерфейс
if ip link show enP3p49s0 >/dev/null 2>&1; then
    # Настраиваем статический IP
    sudo ip addr add 192.168.2.121/24 dev enP3p49s0 2>/dev/null || true
    sudo ip link set enP3p49s0 up 2>/dev/null || true
    
    echo -e "${GREEN}✅ IP 192.168.2.121/24 назначен на enP3p49s0${NC}"
else
    echo -e "${YELLOW}⚠️ Интерфейс enP3p49s0 не найден${NC}"
    echo -e "${YELLOW}   Проверьте имя интерфейса командой: ip link show${NC}"
fi


# ============================================================
# ШАГ 6: Настройка SSH ключей
# ============================================================
echo -e "${BLUE}[6/8] Настройка SSH ключей...${NC}"
...
# ============================================================
# ШАГ 7: Создание run_master.sh
# ============================================================
echo -e "${BLUE}[7/8] Создание run_master.sh...${NC}"
...
echo -e "   ssh-copy-id root@192.168.2.X"
echo -e "   , где X - номер устройства"
